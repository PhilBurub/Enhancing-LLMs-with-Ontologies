...
Первые 2 эпохи обучался только полносвязный слой, затем еще 4 эпохи обучалась вся модель. Для валидации брался тот чекпоинт, на котором самое высокое значение F1 (не accuracy/precision/recall, так как экземпляров класса 0 в датасете в ~4 раза больше, чем класса 1). Обученные модели назовем Sequence Ranker, по аналогии с [Salnikov et al., 2023].
